...
export class DgraphStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const taskDefinition = createDGraphTaskDefinition(
      this,
      executionRole,
      taskRole,
      volume,
      [
        {
          ...
          logging,
          command:
            '/bin/sh -c "dgraph alpha --my=localhost:7080 --zero=localhost:5080 --security whitelist=0.0.0.0/0"',
          ports: [8080, 9080],
    ...
    createService(this, cluster, taskDefinition, fileSystem, [
      {
        name: "DGraphRatel",
        port: 8000,
        containerName: "DGraphRatelContainer",
...
function createDGraphContainer(
  stack: cdk.Stack,
  containerProps: DGraphContainerProps
) {
  const {
    containerName,
    taskDefinition,
...
function createDGraphTaskDefinition(
  ...
  containers.forEach((container) =>
    createDGraphContainer(stack, { ...container, taskDefinition })
  );
...
function registerECSTarget(
  targetRegistrationProps: DGraphTargetRegistrationProps
  ...
  if (targetProps.hostHeader) {
    conditions.push(
      elbv2.ListenerCondition.hostHeaders([targetProps.hostHeader])
    );
  }
  if (targetProps.pathPattern) {
    conditions.push(
      elbv2.ListenerCondition.pathPatterns([targetProps.pathPattern])
    );
  }
  const targetGroup = fargate.listener.addTargets(
    `ECSTargetGroup${containerName}${targetProps.containerPort}`,
    {
      port: port || 80,
      targets: [
...
function createService(
  ...
  taskDefinition: ecs.TaskDefinition,
  fileSystem: efs.FileSystem,
  extraListeners?: ListenerDefinition[]
) {
  ...
  extraListeners?.forEach((def) => {
    const { name, port, containerName } = def;
    const listener = fargate.loadBalancer.addListener(`${name}Listener`, {
      port,
    ...
    listener.addTargets(`${name}ListenerTargetGroup`, {
      protocol: elbv2.ApplicationProtocol.HTTP,
      targets: [target],
    });
  });
  ...
  fargate.service.connections.allowFrom(
    fileSystem,
    ec2.Port.tcp(efs.FileSystem.DEFAULT_PORT)
  );
  fargate.service.connections.allowTo(
    fileSystem,
    ec2.Port.tcp(efs.FileSystem.DEFAULT_PORT)
  );
